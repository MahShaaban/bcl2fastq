...
# set fastq_folders_array paths and runtime parameters
## folder paths should have no slash (/) at end
## fastq folder(s) are where the R1 and R2 FASTQ files are stored for a given experiment
## each fastq folder path should be in quotes, and there should be a space between array entries
## - example: ("/home/groups/MaxsonLab/input-data2/CUTTAG/241213_VH00711_179_AAG5GK5M5_CUTTAG/demultiplexed/241213_HCarlson_CUTTAG" "/home/groups/MaxsonLab/input-data2/CUTTAG/241213_VH00711_179_AAG5GK5M5_CUTTAG/demultiplexed/241213_Exp2_CUTTAG")
...
for fastq_folder in "${fastq_folders_array[@]}"
do
	# sbatch run fastqc for each fastq file, 
	for file in $(find "$fastq_folder" -maxdepth 1 -name "*.gz")
	do
		echo "$file"
